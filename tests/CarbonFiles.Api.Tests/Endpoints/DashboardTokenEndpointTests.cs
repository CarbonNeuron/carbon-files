...
public class DashboardTokenEndpointTests : IClassFixture<TestFixture>
{
    ...
    public async Task CreateToken_AsAdmin_Returns201WithTokenAndExpiresAt()
    {
        ...
        var response = await client.PostAsJsonAsync("/api/tokens/dashboard", new { });
    ...
    {
        using var client = _fixture.CreateAdminClient();
        var response = await client.PostAsJsonAsync("/api/tokens/dashboard", new { expires_in = "6h" });
    ...
    {
        var response = await _fixture.Client.PostAsJsonAsync("/api/tokens/dashboard", new { });
    ...
    public async Task CreateToken_ExceedsCap_Returns400()
    {
        ...
        var response = await client.PostAsJsonAsync("/api/tokens/dashboard", new { expires_in = "3d" });
    ...
    public async Task CreateToken_InvalidFormat_Returns400()
    {
        ...
        var response = await client.PostAsJsonAsync("/api/tokens/dashboard", new { expires_in = "2d" });
    ...
    public async Task CreatedToken_CanAccessAdminEndpoints()
    {
        ...
        var createResponse = await adminClient.PostAsJsonAsync("/api/tokens/dashboard", new { });
    ...
    public async Task ValidateMe_WithValidToken_Returns200()
    {
        ...
        using var dashboardClient = _fixture.CreateAuthenticatedClient(token);
        var meResponse = await dashboardClient.GetAsync("/api/tokens/dashboard/me");
    ...
    {
        var response = await _fixture.Client.GetAsync("/api/tokens/dashboard/me");
    ...
    public async Task ValidateMe_InvalidToken_Returns401()
    {
        ...
        var response = await client.GetAsync("/api/tokens/dashboard/me");
        response.StatusCode.Should().Be(HttpStatusCode.Unauthorized);
